document.addEventListener('DOMContentLoaded', () => {
    // Function to parse front matter from markdown files
    function parseMarkdown(markdown) {
        const parts = markdown.split('---');
        const frontMatter = parts[1];
        const body = parts.slice(2).join('---').trim();
        
        const lines = frontMatter.split('\n').filter(line => line.trim() !== '');
        const data = {};
        lines.forEach(line => {
            const [key, value] = line.split(':').map(part => part.trim());
            data[key] = value.replace(/"/g, ''); // Remove quotes
        });
        
        return { data, body };
    }

    // Function to render markdown to HTML
    function renderMarkdown(markdown) {
        // A simple markdown-to-HTML converter
        // This is a basic example; for a real site, you'd use a library like 'marked.js'
        let html = markdown
            .replace(/^### (.*$)/gim, '<h3>$1</h3>')
            .replace(/^## (.*$)/gim, '<h2>$1</h2>')
            .replace(/^# (.*$)/gim, '<h1>$1</h1>')
            .replace(/\*\*(.*)\*\*/gim, '<strong>$1</strong>')
            .replace(/\*(.*)\*/gim, '<em>$1</em>')
            .replace(/^- (.*$)/gim, '<li>$1</li>')
            .replace(/<\/li>\n<li>/gim, '</li><li>')
            .replace(/(<li>.*<\/li>)/gim, '<ul>$1</ul>')
            .replace(/^(?!<h|<ul|<p|<l).*$/gim, '<p>$&</p>');

        return html;
    }

    // Main logic to fetch and display posts
    const isBlogPage = window.location.pathname.endsWith('blog.html');
    const blogListContainer = document.getElementById('blog-list');
    const blogPostContentContainer = document.getElementById('blog-post-content');

    if (isBlogPage && window.location.hash) {
        // This is the blog page, show a single post
        const slug = window.location.hash.substring(1); // Get post slug from URL
        fetch(`/_blog/${slug}.md`)
            .then(response => response.text())
            .then(markdown => {
                const { data, body } = parseMarkdown(markdown);
                const postHtml = `
                    <div class="blog-post">
                        <h2>${data.title}</h2>
                        <p class="post-meta">${data.date}</p>
                        <div class="post-body">${renderMarkdown(body)}</div>
                    </div>
                `;
                blogPostContentContainer.innerHTML = postHtml;
            })
            .catch(error => {
                blogPostContentContainer.innerHTML = `<p>Post not found.</p>`;
                console.error('Error fetching post:', error);
            });
    } else {
        // This is the index page, show a list of posts
        fetch('/admin/config.yml')
            .then(response => response.text())
            .then(configText => {
                const blogCollectionRegex = /name:\s*"blog"[\s\S]*?folder:\s*"([^"]+)"/;
                const match = configText.match(blogCollectionRegex);
                const folder = match ? match[1] : '_blog';
                
                return fetch(`https://api.github.com/repos/kirat-singh2006/your-repo-name/contents/${folder}`);
            })
            .then(response => response.json())
            .then(files => {
                const markdownFiles = files.filter(file => file.name.endsWith('.md'));
                markdownFiles.forEach(file => {
                    fetch(file.download_url)
                        .then(response => response.text())
                        .then(markdown => {
                            const { data } = parseMarkdown(markdown);
                            const slug = file.name.replace('.md', '');
                            const postLink = `blog.html#${slug}`;
                            const postHtml = `
                                <div class="blog-card">
                                    <h3><a href="${postLink}">${data.title}</a></h3>
                                    <p class="post-date">${data.date}</p>
                                </div>
                            `;
                            blogListContainer.innerHTML += postHtml;
                        });
                });
            })
            .catch(error => console.error('Error fetching blog posts:', error));
    }
});
